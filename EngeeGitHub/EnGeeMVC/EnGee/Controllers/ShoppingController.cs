...
namespace EnGee.Controllers
{
    ...
    {//TODO特定時間清除多餘SESSION
        ...
        //private int GetDeliveryFee(int deliveryTypeId)20231001原先給ConfirmPurchase用，現在疑似用不到
        //{ // 根據送貨類型獲取運費
        //    try
        //    {
        ...
        { // QuickAddToCart用，獲取或更新購物車資料
            ...
            {
                Console.WriteLine($"Error updating the cart: {ex.Message}");
                return new List<SSJ_CShoppingCarItem>();
            }
        }
        ...
        {//ConfirmPurchase用
            ...
            {
                Console.WriteLine($"Error logging the database update exception: {ex.Message}");
            }
        }
        //--------------------------------------
        ...
        {//// overview:
         ...
         //// txtProductId(Key:TempProductId)、 deliverytypeid(Key:TempDeliverytypeid)、 txtCount(Key:TempTxtCount)，最後跳轉至登入頁面
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine($"Error in OnActionExecuting: {ex.Message}");
            }
        }
        ...
        {//Detail中的直接購買(跳轉至cartview)
            ...
            {
                Console.WriteLine($"Error in AddToCartAndReturnCarView: {ex.Message}");
                return View("Error");
                //return RedirectToAction("CartView");
            }
        }
        ...
        { // 購物頁商品快速加入商品到購物車
          // 設定預設的配送選項
            ...
            {
                Console.WriteLine($"Error in QuickAddToCart: {ex.Message}");
                return View("Error"); 
                //return RedirectToAction("CartView");
            }
        }
        ...
        public ActionResult ConfirmPurchase(SSJ_ConfirmPurchaseViewModel vm, string SelectedProducts)
        {
            try
            {
                ...
                {
                    TempData["ErrorMessage_PointInsufficient"] = "您的點數不足"; 
